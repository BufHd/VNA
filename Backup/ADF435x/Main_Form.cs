...
namespace ADF435x
{
    public partial class Main_Form : Form
    {
        ...
        double RFout, REFin, PFDFreq, RFoutMax = 4400, RFoutMin = 34.375, REFinMax = 250, PFDMax = 32, OutputChannelSpacing, N, INT, MOD, FRAC;
        ...
        bool SweepActive = false, HopActive = false, messageShownToUser = false;
        ...
        private void BuildRegisters()
        {
            ...
            PFDFreq = Convert.ToDouble(PFDFreqBox.Text);
            ...
            if (FeedbackSelectBox.SelectedIndex == 1)
                N = (RFout * Convert.ToInt16(OutputDividerBox.Text)) / PFDFreq;
            else
                N = (RFout / PFDFreq);

            INT = (uint)N;
            MOD = (uint)(Math.Round(1000 * (PFDFreq / OutputChannelSpacing)));
            FRAC = (uint)(Math.Round((N - INT) * MOD));
            ...
            RFoutBox.Text = (((INT + (FRAC / MOD)) * PFDFreq / Convert.ToInt16(DivBox.Text)) * ((FeedbackSelectBox.SelectedIndex == 1) ? 1 : Convert.ToInt16(DivBox.Text))).ToString();
            ...
            {
                BandSelectClockFrequencyBox.BackColor = Color.Tomato;
                StatusBarLabel.Text = "Warning! - Band Select Clock Frequency too high. It should be less than 500 kHz.";
            }
            else if ((Convert.ToDouble(BandSelectClockFrequencyBox.Text) > 125) & (BandSelectClockModeBox.SelectedIndex == 1) & (ADF4351.Checked))
            {
                StatusBarLabel.Text = "";
                BandSelectClockFrequencyBox.BackColor = SystemColors.Control;
            }
            else if ((Convert.ToDouble(BandSelectClockFrequencyBox.Text) > 125) & (BandSelectClockModeBox.SelectedIndex == 0) & (ADF4351.Checked))
            {
                StatusBarLabel.Text = "Warning! - Band Select Clock Frequency too high. Reduce to below 125 kHz or set Band Select Clock Mode high.";
                BandSelectClockFrequencyBox.BackColor = Color.Tomato;
            }
            else if ((Convert.ToDouble(BandSelectClockFrequencyBox.Text) > 125) & (ADF4350.Checked))
            {
                BandSelectClockFrequencyBox.BackColor = Color.Tomato;
                StatusBarLabel.Text = "Warning! - Band Select Clock Frequency too high. It should be less than 125 kHz.";
            }
            else
            {
                StatusBarLabel.Text = "";
            ...
            if (CLKDivModeBox.SelectedIndex == 2)
            {
                ...
                TsyncLabel.Text = "Tsync = " + ((1 / PFDFreq) * MOD * Convert.ToInt32(ClockDividerValueBox.Value)).ToString() + " us";
            }
            else
        ...
        {
            MessageBox.Show("Analog Devices ADF435x software - Revision 4.2.7 - January 2013");
        }
        ...
        private void RFOutFreqBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (RFout > RFoutMax)
                {
                    StatusBarLabel.Text = "RF output frequency too high!";
                    RFOutFreqBox.BackColor = Color.Tomato;
                }
                else if (RFout < RFoutMin)
                {
                    StatusBarLabel.Text = "RF output frequency too low!";
                    RFOutFreqBox.BackColor = Color.Tomato;
                }
                else
                {
                    StatusBarLabel.Text = "";
                    RFOutFreqBox.BackColor = Color.White;
                }
            ...
            {
                StatusBarLabel.Text = "Invalid RF output frequency input. Please enter a numeric value.";
            }
        }
        ...
        private void RefFreqBox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                StatusBarLabel.Text = "Invalid reference frequency input. Please enter a numeric value.";
            }
        }

        private void PFDFreqBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                PFDFreq = Convert.ToDouble(PFDFreqBox.Text);

                if ((PFDFreq / 1000) > PFDMax)
                {
                    StatusBarLabel.Text = "PFD frequency too high!";
                    PFDFreqBox.BackColor = Color.Tomato;
                }
                else
                {
                    StatusBarLabel.Text = "";
                    PFDFreqBox.BackColor = Color.White;
                }
            ...
            {
                StatusBarLabel.Text = "Invalid PFD frequency input. Please enter a numeric value.";
            }
        }
        ...
        private void Limit_Check(object sender, EventArgs e)
        {
        ...
        private void FillMainControlsFromRegisters(object sender, EventArgs e)
        {
            ...
            if (ADF4350.Checked)
            {
                ...
                PFDFreq = Convert.ToDouble(RefFreqBox.Text) / (double)RcounterBox.Value * ((RefDoublerBox.Checked) ? 2 : 1) * ((RefD2Box.Checked) ? 0.5 : 1);
                PFDFreqBox.Text = PFDFreq.ToString();
                RFout = PFDFreq * (i + (f / m)) / (Convert.ToDouble(OutputDividerBox.Text));
